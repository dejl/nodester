        ...
        if (err1) {
          ...
          */
          ...
          if (!path.existsSync(path.join(configData.apphome,'app', app.start))) {
            //Bad install??
            log.warn('App start file does not exist: %s', path.join(configData.apphome,'app', app.start));
          ...
          var cmd = 'cd ' + configData.apphome ;
              cmd += ' && sudo -u nodester' + path.join(cfg.app_dir, 'scripts', 'chroot_runner.js');
